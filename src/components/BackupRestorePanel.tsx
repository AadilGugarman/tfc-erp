...
import { Archive, Clock3, Database, Download, FolderOpen, HardDriveUpload, Loader2, RefreshCw, RotateCcw, Search, ShieldCheck, Trash2 } from 'lucide-react';
...
const frequencyOptions: Array<{ value: BackupConfig['frequency']; label: string }> = [
...
export function BackupRestorePanel({ onNotify, onSaveConfig }: BackupRestorePanelProps) {
  ...
  const [sortBy, setSortBy] = useState<'latest' | 'oldest' | 'largest' | 'smallest'>('latest');

  const reload = async () => {
    setLoading(true);
    try {
      const [cfg, backups] = await Promise.all([getBackupConfig(), listBackups()]);
      setConfig(cfg);
      setHistory(backups);
    } catch {
  ...
  const handleRestore = async (item: BackupHistoryItem) => {
    ...
    try {
      ...
      if (result.client_state_json) {
        localStorage.setItem('backup_restore_snapshot', result.client_state_json);
      }
      ...
      if (snapshot) {
        ...
        write('fruit-market-erp-current-company', parsed.current_company_id);
        write('fruit-market-erp-companies', parsed.companies_json);
        write('fruit-market-erp-session', parsed.session_json);
        write('fruit-market-erp-user', parsed.user_json);
        write('fruit_market_erp_db', parsed.preferences_json);
  ...
  const cardClass = 'rounded-xl border border-[#d7e0f2] dark:border-[#25314a] bg-[linear-gradient(145deg,#ffffff,#f7faff)] dark:bg-[linear-gradient(145deg,#0f172a,#111827)] p-4';
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <section className={cardClass}>
          <div className="flex items-center justify-between pb-3 border-b border-[#d9e1ef] dark:border-[#22314a]">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-[13px] font-semibold text-slate-900 dark:text-slate-100">Auto Backup</h3>
            </div>
          ...
          {!config && (
            <p className="text-[12px] text-slate-500 mt-3">Loading backup settings...</p>
          )}
          ...
          {config && (
            <div className="space-y-3 mt-3">
              <label className="flex items-center justify-between rounded-lg border border-[#d3def2] dark:border-[#2a3a58] px-3 py-2 text-[12px]">
                <span className="font-medium text-slate-700 dark:text-slate-300">Enable automatic backup</span>
                <input
                  type="checkbox"
                  checked={config.auto_enabled}
                  onChange={(event) => setConfig((prev) => prev ? { ...prev, auto_enabled: event.target.checked } : prev)}
                />
              </label>

              <label className="flex items-center justify-between rounded-lg border border-[#d3def2] dark:border-[#2a3a58] px-3 py-2 text-[12px]">
                <span className="font-medium text-slate-700 dark:text-slate-300">Create startup backup</span>
                <input
                  type="checkbox"
                  checked={config.startup_backup}
                  onChange={(event) => setConfig((prev) => prev ? { ...prev, startup_backup: event.target.checked } : prev)}
                />
              </label>

              <div>
                <label className="block text-[11px] font-semibold uppercase tracking-[0.06em] text-slate-500 mb-1">Frequency</label>
                <select
                  value={config.frequency}
                  onChange={(event) => setConfig((prev) => prev ? { ...prev, frequency: event.target.value as BackupConfig['frequency'] } : prev)}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-semibold uppercase tracking-[0.06em] text-slate-500 mb-1">Hour</label>
                  <input
                    type="number"
                    min={0}
                    max={23}
                    value={config.backup_hour}
                    onChange={(event) => setConfig((prev) => prev ? { ...prev, backup_hour: Math.max(0, Math.min(23, Number(event.target.value) || 0)) } : prev)}
                    className="w-full h-10 px-3 rounded-xl border border-[#d3def2] dark:border-[#2a3a58] bg-white dark:bg-[#0f172a] text-[13px]"
                  />
                </div>
                <div>
                  <label className="block text-[11px] font-semibold uppercase tracking-[0.06em] text-slate-500 mb-1">Minute</label>
                  <input
                    type="number"
                    min={0}
                    max={59}
                    value={config.backup_minute}
                    onChange={(event) => setConfig((prev) => prev ? { ...prev, backup_minute: Math.max(0, Math.min(59, Number(event.target.value) || 0)) } : prev)}
                    className="w-full h-10 px-3 rounded-xl border border-[#d3def2] dark:border-[#2a3a58] bg-white dark:bg-[#0f172a] text-[13px]"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-semibold uppercase tracking-[0.06em] text-slate-500 mb-1">Retention count</label>
                  <input
                    type="number"
                    min={1}
                    max={3650}
                    value={config.retention_count}
                    onChange={(event) => setConfig((prev) => prev ? { ...prev, retention_count: Math.max(1, Number(event.target.value) || 1) } : prev)}
                    className="w-full h-10 px-3 rounded-xl border border-[#d3def2] dark:border-[#2a3a58] bg-white dark:bg-[#0f172a] text-[13px]"
                  />
                </div>
                <label className="flex items-end justify-between rounded-lg border border-[#d3def2] dark:border-[#2a3a58] px-3 py-2 text-[12px]">
                  <span className="font-medium text-slate-700 dark:text-slate-300">ZIP compression</span>
                  <input
                    type="checkbox"
                    checked={config.compression_enabled}
                    onChange={(event) => setConfig((prev) => prev ? { ...prev, compression_enabled: event.target.checked } : prev)}
                  />
                </label>
              </div>

              <p className="text-[11px] text-slate-500">
                Default path: AppData/TFC ERP/Backups. Latest backup is never auto-deleted.
              </p>
        ...
        <section className={cardClass}>
          <div className="flex items-center gap-2 pb-3 border-b border-[#d9e1ef] dark:border-[#22314a]">
            ...
            <h3 className="text-[13px] font-semibold text-slate-900 dark:text-slate-100">Manual Backup & Restore</h3>
          </div>
          ...
          <div className="space-y-3 mt-3">
            <div className="rounded-lg bg-[#edf3ff] dark:bg-[#11203a] p-3 border border-[#d6e4ff] dark:border-[#2c4263]">
              <p className="text-[12px] text-slate-700 dark:text-slate-300 leading-relaxed">
                Backups include SQLite database, company profiles, language and print preferences, and ERP settings for all companies.
              </p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <Button size="sm" onClick={() => void handleBackupNow()} loading={creatingBackup}>
                <HardDriveUpload className="h-3.5 w-3.5" />
                Backup Now
              </Button>
              <Button size="sm" variant="outline" onClick={() => void openBackupFolder().then(() => onNotify('Backup folder opened.', 'success')).catch(() => onNotify('Could not open backup folder.', 'error'))}>
            ...
            <div className="rounded-lg border border-[#d6e0f2] dark:border-[#2a3a58] p-3">
              <div className="flex items-center justify-between">
                <p className="text-[12px] font-semibold text-slate-800 dark:text-slate-200">Restore Progress</p>
                <p className="text-[11px] tabnum text-slate-500">{restoreProgress}%</p>
              </div>
              <div className="mt-2 h-2 rounded bg-slate-200 dark:bg-[#1b2a43] overflow-hidden">
                <div className="h-full bg-blue-600 transition-all duration-300" style={{ width: `${restoreProgress}%` }} />
              </div>
              <p className="text-[11px] text-slate-500 mt-2">{restoreStage.replaceAll('_', ' ')}</p>
            </div>
          </div>
        </section>
      </div>
      ...
      <section className={cardClass}>
        <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b border-[#d9e1ef] dark:border-[#22314a]">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-[13px] font-semibold text-slate-900 dark:text-slate-100">Backup History</h3>
          </div>
        ...
        {!loading && filteredHistory.length === 0 && (
          <div className="py-8 text-center text-[12px] text-slate-500">No backups found.</div>
        )}
            ...
            <table className="w-full text-[12px]">
              ...
              <tbody>
                {filteredHistory.map((item) => {
                  ...
                  return (
                    <tr key={item.file_path} className="border-b border-[#e8edf8] dark:border-[#1d2a42]">
                      <td className="py-2 pr-2 text-slate-700 dark:text-slate-300">
                        <div>{formatBackupDate(item.created_at)}</div>
                        <div className="text-[10px] text-slate-500">{item.file_name}</div>
                      </td>
                      ...
                      <td className="py-2 pr-2 tabnum text-slate-700 dark:text-slate-300">{formatBytes(item.size_bytes)}</td>
                      <td className="py-2 pr-2 tabnum text-slate-700 dark:text-slate-300">{item.company_count}</td>
                      <td className="py-2">
                        <div className="flex flex-wrap gap-1.5">
                          <Button size="xs" variant="soft" onClick={() => void handleRestore(item)} disabled={restoring}>
                            {restoring ? <Loader2 className="h-3 w-3 animate-spin" /> : <RotateCcw className="h-3 w-3" />}
                            Restore
                          </Button>
                          <Button size="xs" variant="outline" onClick={() => void handleExport(item)}>
                            <Download className="h-3 w-3" />
                            Export
                          </Button>
                          <Button size="xs" variant="outline" className="text-red-600 border-red-200 hover:bg-red-50 dark:hover:bg-red-950/35" onClick={() => void handleDelete(item)}>
