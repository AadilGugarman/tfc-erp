...
import type { Bill, Company } from "@/db/schema";
...
export function InvoiceViewer({ bill, currentCompany }: InvoiceViewerProps) {
          ...
          <html>
            <head>
              ...
              <style>
                ...
                .invoice-container { width: 80mm; margin: 0 auto; padding: 10mm; border: 1px solid #ccc; }
              ...
              <div class="invoice-container">
                ...
                <div class="details">
                  ...
                  <div>
                    ...
                    ${bill.partyName}<br/>
                    ${bill.partyId ? "Mobile: " + (useAppStore.getState().parties.find(p => p.id === bill.partyId)?.phone || "") : ""}
                  </div>
                </div>
                <div class="items">
                  <table>
                ...
                <div class="totals">
                  <p><strong>Today Invoice Amount:</strong> ₹${bill.total.toLocaleString()}</p>
                  <p><strong>Add: Previous Outstanding:</strong> ₹${bill.previousBalance.toLocaleString()}</p>
                  <p><strong>Less: Cash/UPI Paid Now:</strong> ₹${bill.paidAmount.toLocaleString()}</p>
                  <p><strong>Remaining Due Balance:</strong> ₹${bill.netBalance.toLocaleString()}</p>
                </div>
                <div class="footer">
  ...
  const handleShareOnWhatsApp = () => {
    const message = `Invoice No: ${bill.billNo}\nDate: ${formatDate(bill.date)}\nBilled To: ${bill.partyName}\nTotal Amount: ₹${bill.total.toLocaleString()}\nPaid Now: ₹${bill.paidAmount.toLocaleString()}\nBalance Due: ₹${bill.netBalance.toLocaleString()}\n\nItems:\n${bill.items.map(item => `- ${item.fruitName} (${item.boxCount} C) - ${item.totalWeight} kg @ ₹${item.rate} = ₹${(item.rate * item.totalWeight).toLocaleString()}`).join("\n")}\n\nBank Details: ${currentCompany?.bankName || ""}, A/C: ${currentCompany?.bankAccountNumber || ""}, IFSC: ${currentCompany?.bankIfsc || ""}\nUPI ID: ${currentCompany?.upiId || ""}\n\n${currentCompany?.name || "Your Company"}\n${currentCompany?.phone || ""}`;
      ...
      <div ref={invoiceRef} className="invoice-content">
        ...
        <div className="flex justify-between items-start mb-6">
          ...
          <div className="text-right">
            <h3 className="text-lg font-semibold text-slate-900 dark:text-white">TAX INVOICE</h3>
            <p className="text-sm text-slate-600 dark:text-slate-400">
              Bill No: {bill.billNo}
            </p>
            <p className="text-sm text-slate-600 dark:text-slate-400">
        ...
        <div className="mb-6 p-4 bg-slate-50 dark:bg-slate-800 rounded-lg">
          <p className="text-xs uppercase font-bold text-slate-500 dark:text-slate-400">BILLED TO:</p>
          <h4 className="text-base font-bold text-slate-900 dark:text-white">{bill.partyName}</h4>
          <p className="text-sm text-slate-600 dark:text-slate-400">
            Mobile No: {useAppStore.getState().parties.find(p => p.id === bill.partyId)?.phone || ""}
          </p>
        </div>
        ...
        <div className="flex justify-between mb-6">
          <div className="w-1/2">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400 mt-4">Notes: {bill.notes || "None"}</p>
          </div>
          <div className="w-1/2 text-right">
            <p className="text-base text-slate-700 dark:text-slate-300">Today Invoice Amount: <span className="font-bold">₹{bill.total.toLocaleString()}</span></p>
            <p className="text-sm text-slate-600 dark:text-slate-400">Add: Previous Outstanding: <span className="font-medium">₹{bill.previousBalance.toLocaleString()}</span></p>
            <p className="text-sm text-slate-600 dark:text-slate-400">Less: Cash/UPI Paid Now: <span className="font-medium">₹{bill.paidAmount.toLocaleString()}</span></p>
            <p className="text-lg font-bold text-emerald-600 dark:text-emerald-400">Remaining Due Balance: ₹{bill.netBalance.toLocaleString()}</p>
          </div>
        </div>
