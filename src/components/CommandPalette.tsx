...
import { listCommands, listRegisteredShortcuts } from '@/keyboard/shortcutManager';
...
const PAGE_COMMANDS: Array<{ id: PageId; title: string; shortcut?: string }> = [
  ...
  { id: 'vehicle-register', title: 'Go to Vehicle Register', shortcut: 'Alt+V' },
  { id: 'billing', title: 'Go to Billing', shortcut: 'Alt+B' },
  { id: 'purchases', title: 'Go to Purchases', shortcut: 'Alt+P' },
...
export function CommandPalette({ open, onClose }: CommandPaletteProps) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-start justify-center pt-[12vh] px-4" role="dialog" aria-modal="true" aria-label="Command Palette">
      ...
      <div className="relative w-full max-w-3xl rounded-2xl border border-slate-200/80 dark:border-slate-700/80 bg-white/95 dark:bg-slate-900/95 shadow-2xl overflow-hidden backdrop-blur-xl">
        ...
        <div className="max-h-[45vh] overflow-y-auto divide-y divide-slate-200 dark:divide-slate-700">
          {items.length === 0 ? (
            <div className="px-4 py-10 text-center text-sm text-slate-500">No commands found</div>
          ) : (
            items.map((item, index) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-slate-500">{item.subtitle ?? item.group}</p>
                </div>
                {item.shortcut ? (
        ...
        <div className="px-4 py-3 border-t border-slate-200 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-900/70">
          <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500 mb-2">Global Shortcuts</p>
          <div className="flex flex-wrap gap-2">
            {shortcutHints.slice(0, 8).map((hint) => (
              <span key={hint.id} className="inline-flex items-center gap-1 rounded bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-2 py-1 text-[11px] text-slate-600 dark:text-slate-300" title={hint.description}>
                <kbd className="font-mono text-[10px] text-slate-500">{hint.combo}</kbd>
