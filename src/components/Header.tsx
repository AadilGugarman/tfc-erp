...
import { Moon, Sun, Search, LogOut, ChevronDown, Building2, Bell, CheckCircle2 } from 'lucide-react';
...
function MangoLogo() {
  return (
    <svg viewBox="0 0 32 32" className="h-5 w-5" aria-hidden="true">
      ...
      <path d="M18.5 7.4c-4.6 0-9.8 2.8-11 8.6-1 5 2.2 9.2 7.2 10.2 5.8 1.1 11.7-2.3 12.7-8.7.8-4.8-2.6-10.1-8.9-10.1z" fill="url(#headerMangoBody)" />
      <path d="M19.5 5.4c1.7-2.6 4.3-3.6 7.3-3.1-1.3 2.7-3.6 4.3-6.8 4.8z" fill="url(#headerMangoLeaf)" />
      <path d="M18.2 4.5c.5-1.2 1.3-2.1 2.5-2.8" stroke="#14532d" strokeWidth="1.4" strokeLinecap="round" />
...
export function Header() {
  const { settings, setDarkMode, setCurrentPage, logout, currentCompany, companies, setCurrentCompany, currentPage } = useAppStore();
  ...
  const moduleMeta: Record<string, { title: string; subtitle: string }> = {
    ...
    'vehicle-register': { title: 'Vehicle Register', subtitle: 'Arrival Operations' },
    ...
    billing: { title: 'Billing', subtitle: 'Sales Ledger' },
    purchases: { title: 'Purchases', subtitle: 'Purchase Ledger' },
  ...
  const activeModule = moduleMeta[currentPage] ?? { title: 'Workspace', subtitle: 'Operations' };

  return (
    <header className="fixed top-0 left-0 right-0 z-40 h-16 grid grid-cols-[auto_1fr_auto] items-center gap-3 px-4 sm:px-5 bg-gradient-to-b from-white/88 to-white/72 dark:from-[#0c1528]/90 dark:to-[#0b1222]/72 backdrop-blur-xl border-b border-slate-200/70 dark:border-[#1f2a43]/80 shadow-[0_8px_20px_-18px_rgba(15,23,42,0.55)]">
      ...
      <div className="flex items-center gap-2.5 min-w-0">
        ...
        >
          ...
          <span className="min-w-0 text-left hidden sm:block">
            <span className="block text-[11px] uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400 leading-none">TFC</span>
            <span className="block text-sm font-semibold text-slate-900 dark:text-slate-100 leading-tight group-hover:text-blue-700 dark:group-hover:text-blue-300 transition-colors">Home</span>
          </span>
        </button>
      ...
      <div className="flex justify-center px-2 sm:px-4">
        <button
          ...
          className={cn(
            'w-full max-w-[520px] flex items-center gap-2.5 h-9 px-3 rounded-xl',
        ...
        >
          ...
          <span className="text-sm flex-1 text-left truncate">Search across ERP modules...</span>
          <kbd className="erp-kbd hidden md:flex gap-0.5">
            <span>Ctrl</span>+F
          </kbd>
        </button>
      </div>
      ...
      <div className="flex items-center justify-end gap-1.5 shrink-0">
        <button
          className={cn(
            ...
            'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200',
            'bg-white/65 dark:bg-[#101a31]/80 ring-1 ring-slate-200/70 dark:ring-[#2a3550]/75',
        ...
        <div
          className={cn(
            'hidden md:inline-flex items-center gap-1.5 h-9 px-2.5 rounded-xl text-xs font-medium',
            'text-emerald-700 dark:text-emerald-300',
            'bg-emerald-50/85 dark:bg-emerald-950/30 ring-1 ring-emerald-200/70 dark:ring-emerald-800/45'
          )}
          title="Sync status"
        >
        ...
        <div className="relative" ref={profileRef}>
          ...
          >
            <div className="h-7 w-7 rounded-lg bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center shadow-md shadow-blue-600/30">
