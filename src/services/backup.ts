...
export type BackupFrequency = 'daily' | 'every_12_hours' | 'every_6_hours' | 'startup_only';
...
export function collectClientStateSnapshot(): string {
  const payload: ClientStateSnapshot = {
    ...
    current_company_id: localStorage.getItem('fruit-market-erp-current-company'),
    companies_json: localStorage.getItem('fruit-market-erp-companies'),
    session_json: localStorage.getItem('fruit-market-erp-session'),
    user_json: localStorage.getItem('fruit-market-erp-user'),
    preferences_json: localStorage.getItem('fruit_market_erp_db'),
  };
...
export function applyClientStateSnapshot(snapshotJson: string): void {
  ...
  setOrRemove('fruit-market-erp-current-company', parsed.current_company_id);
  setOrRemove('fruit-market-erp-companies', parsed.companies_json);
...
export async function onRestoreProgress(handler: (payload: { progress: number; stage: string }) => void): Promise<UnlistenFn> {
  if (!isDesktopRuntime()) {
    return () => {};
  }
  return listen<{ progress: number; stage: string }>('backup-restore-progress', (event) => handler(event.payload));
}
