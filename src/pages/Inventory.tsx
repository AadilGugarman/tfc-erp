...
export function InventoryPage() {
  ...
  const handleDoInward = async () => {
    ...
    try {
      ...
      success(
        'Stock Added',
        `${inwardQty} units added to inventory`
      );
      setShowInward(false);
      setInwardItem('');
      setInwardQty(0);
    ...
    <PageTransition>
      ...
      <PageLayout
        ...
        actions={
          <div className="flex items-center gap-2">
            <Button variant="outline" icon={<ArrowUpCircle size={16} />} onClick={() => setShowInward(true)}>
              Stock In
            </Button>
            <Button variant="outline" icon={<History size={16} />} onClick={() => setShowTxn(true)}>
              Transactions
            </Button>
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                  {filtered.map(item => (
                    <PremiumTableRow key={item.id}>
                      <PremiumTableCell className="font-medium">{item.name}</PremiumTableCell>
                      ...
                      <PremiumTableCell>{formatDate(item.lastUpdated)}</PremiumTableCell>
                      <PremiumTableCell>
      ...
      >
        <div className="space-y-5">
          ...
          <PremiumInput
            ...
            error={inwardQty <= 0 && inwardQty !== 0 ? 'Quantity must be greater than 0' : undefined}
          />
      ...
      >
        <div className="space-y-2 max-h-[60vh] overflow-y-auto">
          {txnData.length === 0 ? (
            <div className="text-center py-8 text-slate-400">No transactions</div>
          ) : (
            txnData
              ...
              .map(txn => (
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-slate-500">@ {formatCurrency(txn.rate)}</p>
                  </div>
                </div>
              ))
          )}
