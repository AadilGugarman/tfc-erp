...
function createEmptyLineItem(id = `item-${Date.now()}-${Math.random()}`): LineItem {
  return { id, itemName: '', lotNo: '', crate: 0, weight: 0, rate: 0, amount: 0 };
}
...
export function SalesAndPurchasePage() {
  ...
  const filteredParties = useMemo(() => {
    ...
    return currentPartyList.filter(p => 
  ...
  const handleItemChange = (itemId: string, field: keyof LineItem, value: any) => {
    setFormData(prev => ({
      ...
      items: prev.items.map(item => {
        ...
        const updated = { ...item, [field]: field === 'itemName' || field === 'lotNo' ? value : parseFloat(value) || 0 };
  ...
  const handleSave = (mode: 'draft' | 'save' | 'print') => {
    ...
    if (activeTab === 'sales') {
      ...
      const purchaseData: Omit<Purchase, 'id' | 'purchaseNo' | 'createdAt' | 'updatedAt'> = {
        date: formData.date,
      ...
      <body>
        <div class="container">
          ...
          <table>
            ...
            <tbody>
              ${bill.items.map((item) => {
                const weight = 'totalWeight' in item ? (item as BillItem).totalWeight : (item as PurchaseItem).quantity || 0;
                return `
                <tr>
  ...
  const filteredList = currentList.filter(item => {
    const q = searchTerm.toLowerCase();
    const no = activeTab === 'sales' ? (item as Bill).billNo : (item as Purchase).purchaseNo;
    const party = activeTab === 'sales' ? (item as Bill).partyName : (item as Purchase).supplierName;
  ...
  return (
    <div className="flex flex-col h-full animate-fade-in">
      <PageLayout
        title={activeTab === 'sales' ? 'Sales Bills' : 'Purchase Orders'}
        subtitle={activeTab === 'sales' ? 'Create and manage sales invoices' : 'Create and manage purchase orders'}
      >
        {viewMode === 'list' ? (
          // LIST VIEW
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <Button onClick={() => { setViewMode('form'); setFormData({ ...formData, invoiceNo: getNextInvoiceNo(activeTab === 'sales') }); }}>
                <Plus className="h-4 w-4" /> New {activeTab === 'sales' ? 'Sale' : 'Purchase'}
              </Button>
            </div>
                  ...
                  {filteredList.length === 0 ? (
                    <tr>
                      <td colSpan={8} className="px-4 py-8 text-center text-slate-500">
                        No {activeTab === 'sales' ? 'sales bills' : 'purchase orders'} found
                      </td>
                    </tr>
                  ) : (
                    filteredList.map((item) => {
                      const isBill = activeTab === 'sales';
                      const invoiceNo = isBill ? (item as Bill).billNo : (item as Purchase).purchaseNo;
                      const partyName = isBill ? (item as Bill).partyName : (item as Purchase).supplierName;
                      
                      return (
                        <tr key={item.id} className="border-b border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                          <td className="px-4 py-3 font-mono font-semibold">{invoiceNo}</td>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg p-6 border border-slate-200 dark:border-slate-700 space-y-4">
              ...
              <div className="relative">
                      ...
                      <div className="max-h-48 overflow-y-auto">
                        {filteredParties.map(party => (
                          ...
                          >
                            ...
                            <div className="text-xs text-slate-600 dark:text-slate-400">{party.phone}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                {selectedParty && (
                  <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg text-sm space-y-1">
                    <div><strong>Phone:</strong> {selectedParty.phone}</div>
                    <div><strong>City:</strong> {selectedParty.city}</div>
                    <div><strong>GST:</strong> {(selectedParty as any).gstin || 'N/A'}</div>
                    <div><strong>Balance:</strong> {formatCurrency(selectedParty.openingBalance)}</div>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">LR Number</label>
                  <input
                    type="text"
                    value={formData.lrNo}
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {formData.items.map((item) => (
                      <tr key={item.id} className="border border-slate-300 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700">
                        ...
                        <td className="px-3 py-2">
                          <input
                            type="number"
                            value={item.crate || ''}
                            onChange={(e) => handleItemChange(item.id, 'crate', e.target.value)}
                            placeholder="0"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-white dark:bg-slate-800 rounded-lg p-6 border border-slate-200 dark:border-slate-700 space-y-4">
                ...
                <div className="flex justify-between items-center pb-2 border-b border-slate-300 dark:border-slate-600">
                  ...
                  <span className="font-mono text-lg">{formatCurrency(itemsTotal)}</span>
                </div>

                <div className="flex justify-between items-center pb-2 border-b border-slate-300 dark:border-slate-600">
                  <span className="text-slate-600 dark:text-slate-400">Previous Balance:</span>
                  <span className="font-mono">{formatCurrency(selectedParty?.openingBalance || 0)}</span>
                </div>
                ...
                <div className="flex justify-between items-center text-lg font-bold pt-2 border-t-2 border-slate-300 dark:border-slate-600">
                  ...
                  <span className="font-mono text-blue-600 dark:text-blue-400">{formatCurrency(grandTotal)}</span>
                </div>

                <div className="flex justify-between items-center text-lg font-bold pt-2 border-t-2 border-slate-300 dark:border-slate-600">
                  <span>Net Balance:</span>
                  <span className="font-mono text-green-600 dark:text-green-400">{formatCurrency(netBalance)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-3 justify-end">
              ...
              <Button className="bg-green-600 hover:bg-green-700" onClick={() => handleSave('print')}>
                Save & Print
              </Button>
            </div>
          </div>
